...
import type { GimbalState } from '../types/gimbal';

interface GimbalViewerProps {
  state: GimbalState | null;
}

export default function GimbalViewer({ state }: GimbalViewerProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const sceneRef = useRef<THREE.Scene | null>(null);
  const cameraRef = useRef<THREE.PerspectiveCamera | null>(null);
  const rendererRef = useRef<THREE.WebGLRenderer | null>(null);
  const azimuthGroupRef = useRef<THREE.Group | null>(null);
  const elevationGroupRef = useRef<THREE.Group | null>(null);
  const rollGroupRef = useRef<THREE.Group | null>(null);
  const animationRef = useRef<number>(0);
  const targetStateRef = useRef<GimbalState | null>(null);
  ...
  useEffect(() => {
    ...
    const rearCap = new THREE.Mesh(
      new THREE.ConeGeometry(0.4, 0.3, 24),
      new THREE.MeshStandardMaterial({
        color: 0x5a6a8a,
        metalness: 0.7,
        roughness: 0.35,
    ...
    const azimuthIndicator = new THREE.Mesh(
